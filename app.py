...
SHIFT_OPTIONS = ["", "10:00-18:00", "18:00-23:00", "10:00-23:00", "10:00-15:00",
                 "11:00-18:00", "17:00-23:00", "18:30-23:00", "19:00-23:00"]
...
req_staff_config = load_sheet_cached(
    "required_staff",
    pd.DataFrame(2, index=TIME_SLOTS,
                 columns=[f"{g}_{p}" for g in ["月火水木","金土","日"] for p in ["ホール","キッチン"]])
)
...
# --- ② 従業員設定 ---
    ...
    if not is_admin: st.error("管理者用パスワードが必要です")
        ...
        if st.button("💾 名簿を保存"):
            if save_sheet_robust(edited, "staff_master"):
                st.success("保存完了。ブラウザを更新して反映してください。")
    ...
    if not is_admin: st.error("管理者パスワードを入力してください")
            ...
            for i, key in enumerate(g_keys):
                with tabs[i]:
                    ...
                    if c3.button("全時間に適用", key=f"ap_{key}"):
                        curr_req[f"{key}_ホール"] = bh
                        curr_req[f"{key}_キッチン"] = bk
                        save_sheet_robust(curr_req, "required_staff")
                        st.rerun()
                    for t in TIME_SLOTS:
                        cols = st.columns([1, 2, 2])
                        cols[0].write(f"**{t}**")
                        curr_req.at[t, f"{key}_ホール"] = cols[1].number_input(
                            "H", 0, 10, int(curr_req.at[t, f"{key}_ホール"]),
                            key=f"nh_{key}_{t}", label_visibility="collapsed")
                        curr_req.at[t, f"{key}_キッチン"] = cols[2].number_input(
                            "K", 0, 10, int(curr_req.at[t, f"{key}_キッチン"]),
                            key=f"nk_{key}_{t}", label_visibility="collapsed")
            if st.button("💾 人数設定を保存"):
                save_sheet_robust(curr_req, "required_staff")
        ...
        with col_a1:
            if st.button("🤖 公平自動生成実行"):
                ...
                target_counts = {n: (staff_info[n]['週希望'] if staff_info[n]['週希望'] > 0 else 1) * 4.3
                                 for n in ALL_NAMES}
                for i, col in enumerate(column_names):
                    wd_jp = WEEKDAYS_JP[calendar.weekday(year, month, i + 1)]
                    g_key = ("月火水木" if wd_jp in ["月","火","水","木"]
                             else ("金土" if wd_jp in ["金","土"] else "日"))
                    try:
                    ...
                    capable.sort(key=lambda n: (
                        work_counts[n] / target_counts[n] if target_counts[n] > 0 else 99,
                        random.random()))
                    ...
                    for n in capable:
                        ...
                        if staff_info[n]['レジ締め'] and ef >= 23 and not has_cl:
                            ...
                            if ("☕" in role or "👔" in role) and hd_c < thd:
                                new_s.at[n, col] = f"{tst}-18:00"; work_counts[n] += 1; hd_c += 1
                            elif ("🍳" in role or "👔" in role) and kd_c < tkd:
                                new_s.at[n, col] = f"{tst}-18:00"; work_counts[n] += 1; kd_c += 1
                        elif ef >= 23:
                            if ("☕" in role or "👔" in role) and hn_c < thn:
                                new_s.at[n, col] = f"{tst}-23:00"; work_counts[n] += 1; hn_c += 1
                            elif ("🍳" in role or "👔" in role) and kn_c < tkn:
                                new_s.at[n, col] = f"{tst}-23:00"; work_counts[n] += 1; kn_c += 1
        ...
        with col_a2:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                ws.set_column(1, len(column_names), 12, fmt)
                for c_idx, val in enumerate(current_df.columns):
                    if "(土)" in val:
                        ws.write(0, c_idx + 1, val,
                                 wb.add_format({'bold': True, 'bg_color': '#CCE5FF',
                                                'font_color': '#0000FF', 'border': 1}))
                    elif "(日)" in val:
                        ws.write(0, c_idx + 1, val,
                                 wb.add_format({'bold': True, 'bg_color': '#FFCCCC',
                                                'font_color': '#FF0000', 'border': 1}))
                ws.freeze_panes(1, 1)
            # key= を明示することで ID の衝突を確実に防ぐ
            st.download_button(
                label="📥 Excel保存",
        ...
        edited = st.data_editor(
            ...
            column_config={c: st.column_config.SelectboxColumn(options=SHIFT_OPTIONS, width="medium")
