...
def save_sheet(df, worksheet_name):
    """データを保存する（最もエラーが起きにくいシンプルな方式）"""
    try:
        # スタイル情報を削ぎ落として純粋なデータのみにする
        if hasattr(df, 'data'):
            df = df.data
        ...
        # 保存実行（もしタブがない場合はエラーになるので、その際は手動作成を促す）
        conn.update(spreadsheet=SPREADSHEET_URL, worksheet=worksheet_name, data=save_df)
        st.cache_data.clear()
        return True
    except Exception as e:
        st.error(f"【保存エラー】スプレッドシートに '{worksheet_name}' というタブ（シート）がない可能性があります。")
        st.info("スプレッドシートの下にある『＋』ボタンで、新しいタブを作り、名前を正確に設定してください。")
        return False
...
def init_system_data():
    ...
    all_names_list = master_df['表示名'].unique().tolist()
    staff_info_dict = master_df.set_index('表示名').to_dict('index')

    return master_df, all_names_list, staff_info_dict
...
if mode == "① 休み希望入力":
    ...
    edited_req = st.data_editor(req_df, use_container_width=True, height=700)
    if st.button("💾 希望を保存"):
        if save_sheet(edited_req, REQ_SHEET_NAME): st.success("保存完了")
...
# --- ③ 曜日別バー ---
    ...
    if not is_admin: st.error("管理者用です")
        ...
        for wd in WEEKDAYS_JP:
            ...
            res = st.slider(f"{wd}曜日", 10.0, 23.0, (float(sf), float(ef)), step=0.5, format="%g時", key=f"s_{target}_{wd}")
    ...
    if not is_admin: st.error("管理者パスワードを入力してください")
        ...
        with col_a2:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                base_f = workbook.add_format({'border': 1, 'align': 'center'})
                name_f = workbook.add_format({'bold': True, 'border': 1, 'bg_color': '#F2F2F2'})
                header_f = workbook.add_format({'bold': True, 'border': 1, 'align': 'center', 'bg_color': '#D9D9D9'})
                sat_f = workbook.add_format({'bold': True, 'border': 1, 'align': 'center', 'bg_color': '#CCE5FF', 'font_color': '#0000FF'})
                sun_f = workbook.add_format({'bold': True, 'border': 1, 'align': 'center', 'bg_color': '#FFCCCC', 'font_color': '#FF0000'})
                worksheet.set_column(0, 0, 25, name_f)
                worksheet.set_column(1, len(column_names), 12, base_f)
                for c_idx, val in enumerate(current_df.columns):
                    if "(土)" in val: worksheet.write(0, c_idx + 1, val, sat_f)
                    elif "(日)" in val: worksheet.write(0, c_idx + 1, val, sun_f)
                    else: worksheet.write(0, c_idx + 1, val, header_f)
                worksheet.freeze_panes(1, 1)
            st.download_button(label="📥 見やすいExcel保存", data=output.getvalue(), file_name=f"shift_{year}_{month:02}.xlsx")
